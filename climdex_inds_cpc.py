...
This script calculates the following from CPC data: 
...
tmmx_path = "/data/singh/data/cpc_temp/tmax/"
tmmn_path = "/data/singh/data/cpc_temp/tmin/"

## Read in and sort the matching files
infiles_max = sorted(glob.glob(tmmx_path + "*199[1-9]*.nc")) + sorted(glob.glob(tmmx_path + "*20[0-2][0-9]*.nc"))
infiles_min = sorted(glob.glob(tmmn_path + "*199[1-9]*.nc")) + sorted(glob.glob(tmmn_path + "*20[0-2][0-9]*.nc"))
...
for max_entry, min_entry in zip(infiles_max, infiles_min):
    ...
    ## Latitude and longitude boundaries for the CONUS
    min_lon = -125.25
    min_lat = 50.25
    max_lon = -68.75
    max_lat = 24.75
    ...
    ## Mask outside US because dataset is global
    mask_lon = (ds_max.lon >= min_lon) & (ds_max.lon <= max_lon)
    mask_lat = (ds_max.lat >= min_lat) & (ds_max.lat <= max_lat)
    ds_max = ds_max.sel(lat=slice(min_lat, max_lat), lon=slice(min_lon, max_lon))
    
    mask_lon = (ds_min.lon >= min_lon) & (ds_min.lon <= max_lon)
    mask_lat = (ds_min.lat >= min_lat) & (ds_min.lat <= max_lat)
    ds_min = ds_min.sel(lat=slice(min_lat, max_lat), lon=slice(min_lon, max_lon))
    
    ## Rename temperature dimension from day to time to match gridMET        
    ds_max = ds_max.rename(name_dict={'tmax' : 'air_temperature'})
    ds_min = ds_min.rename(name_dict={'tmin' : 'air_temperature'})
...
avg_t_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_T_1991_2020.nc')
...
avg_t_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_T_trends_1991_2020.nc')

TNn_concat = xr.concat(TNn_list, dim=concat_var)
TNn_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_min_T_1991_2020.nc')
TNn_trends = TNn_concat.polyfit(dim=concat_var, deg=1, skipna=True)
TNn_trends_a = TNn_trends.polyfit_coefficients.sel(degree=1)
TNn_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_TNn_trends_1991_2020.nc')

TXx_concat = xr.concat(TXx_list, dim=concat_var)
TXx_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_max_T_1991_2020.nc')
TXx_trends = TXx_concat.polyfit(dim=concat_var, deg=1, skipna=True)
TXx_trends_a = TXx_trends.polyfit_coefficients.sel(degree=1)
TXx_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_TXx_trends_1991_2020.nc')

T_range_concat = xr.concat(T_range_list, dim=concat_var)
T_range_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_T_range_1991_2020.nc')
...
T_range_concat_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_T_range_trends_1991_2020.nc')

grow_seas_concat = xr.concat(grow_seas_list, dim=concat_var)
grow_seas_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_grow_seas_1991_2020.nc')
...
grow_seas_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_grow_seas_trends_1991_2020.nc')

frost_days_concat = xr.concat(frost_days_list, dim=concat_var)
frost_days_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_avg_annual_frost_days_1991_2020.nc')
...
frost_days_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_frost_days_trends_1991_2020.nc')

summer_days_concat = xr.concat(summer_days_list, dim=concat_var)
summer_days_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_annual_summer_days_1991_2020.nc')
...
summer_days_concat_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_summer_days_trends_1991_2020.nc')

x_mo_range_concat = xr.concat(x_mo_range_list, dim=concat_var)
x_mo_range_concat.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_annual_ext_month_range_1991_2020.nc')
...
x_mo_range_concat_trends_a.to_netcdf('/data/singh/yianna/butterfly_nc_files/cpc_ext_month_range_trends_1991_2020.nc')
